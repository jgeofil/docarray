...
def dunder_get(_dict: Any, key: str) -> Any:
    ...
    if (
        ...
        or not isinstance(_dict, dict)
        and isinstance(_dict, Sequence)
    ):
        result = _dict[part1]
...
def get_compress_ctx(algorithm: Optional[str] = None, mode: str = 'wb'):
    if algorithm == 'lz4':
        import lz4.frame
        return lambda x: lz4.frame.LZ4FrameFile(x, mode)
    elif algorithm == 'gzip':
        import gzip
        return lambda x: gzip.GzipFile(fileobj=x, mode=mode)
    elif algorithm == 'bz2':
        import bz2
        return lambda x: bz2.BZ2File(x, mode)
    elif algorithm == 'lzma':
        import lzma
        return lambda x: lzma.LZMAFile(x, mode)
    else:
        return None
